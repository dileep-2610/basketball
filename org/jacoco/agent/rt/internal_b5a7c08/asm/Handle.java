/*     */ package org.jacoco.agent.rt.internal_b5a7c08.asm;
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ public final class Handle
/*     */ {
/*     */   private final int tag;
/*     */   private final String owner;
/*     */   private final String name;
/*     */   private final String descriptor;
/*     */   private final boolean isInterface;
/*     */   
/*     */   @Deprecated
/*     */   public Handle(int tag, String owner, String name, String descriptor) {
/*  76 */     this(tag, owner, name, descriptor, (tag == 9));
/*     */   }
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   
/*     */   public Handle(int tag, String owner, String name, String descriptor, boolean isInterface) {
/*  99 */     this.tag = tag;
/* 100 */     this.owner = owner;
/* 101 */     this.name = name;
/* 102 */     this.descriptor = descriptor;
/* 103 */     this.isInterface = isInterface;
/*     */   }
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   
/*     */   public int getTag() {
/* 115 */     return this.tag;
/*     */   }
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   
/*     */   public String getOwner() {
/* 125 */     return this.owner;
/*     */   }
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   
/*     */   public String getName() {
/* 134 */     return this.name;
/*     */   }
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   
/*     */   public String getDesc() {
/* 143 */     return this.descriptor;
/*     */   }
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   
/*     */   public boolean isInterface() {
/* 152 */     return this.isInterface;
/*     */   }
/*     */ 
/*     */   
/*     */   public boolean equals(Object object) {
/* 157 */     if (object == this) {
/* 158 */       return true;
/*     */     }
/* 160 */     if (!(object instanceof Handle)) {
/* 161 */       return false;
/*     */     }
/* 163 */     Handle handle = (Handle)object;
/* 164 */     return (this.tag == handle.tag && this.isInterface == handle.isInterface && this.owner
/*     */       
/* 166 */       .equals(handle.owner) && this.name
/* 167 */       .equals(handle.name) && this.descriptor
/* 168 */       .equals(handle.descriptor));
/*     */   }
/*     */ 
/*     */   
/*     */   public int hashCode() {
/* 173 */     return this.tag + (
/* 174 */       this.isInterface ? 64 : 0) + this.owner
/* 175 */       .hashCode() * this.name.hashCode() * this.descriptor.hashCode();
/*     */   }
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   
/*     */   public String toString() {
/* 188 */     return stringConcat$0(this.owner, this.name, this.descriptor, this.tag, this.isInterface ? " itf" : "");
/*     */   }
/*     */ }


/* Location:              /Users/mac/Downloads/jacocoagent.jar!/org/jacoco/agent/rt/internal_b5a7c08/asm/Handle.class
 * Java compiler version: 5 (49.0)
 * JD-Core Version:       1.1.3
 */